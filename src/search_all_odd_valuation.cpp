// Exact exhaustive search using all p == 3 (mod 4), with a selectable
// allowed valuation family: exactly 1; 1 or 3; or every positive odd valuation.
//
...
enum class ValuationMode { ExactOne, OneOrThree, AnyOdd };

bool allowed_exponent(unsigned e, ValuationMode mode) {
    if (mode == ValuationMode::ExactOne) return e == 1;
    if (mode == ValuationMode::OneOrThree) return e == 1 || e == 3;
    return (e & 1U) != 0;
}

const char* mode_name(ValuationMode mode) {
    if (mode == ValuationMode::ExactOne) return "exact-one";
    if (mode == ValuationMode::OneOrThree) return "one-or-three";
    return "any-odd";
}
...
std::vector<uint64_t> classify_interval(uint64_t A, uint64_t B,
                                        ...
                                        ValuationMode mode, FactorStats& stats) {
    ...
    for (uint32_t p32 : primes) {
        ...
        for (;;) {
            ...
            if (!get_bit(good, static_cast<uint64_t>(idx))) {
                uint64_t quotient = m / p;
                unsigned exponent = 1;
                while (quotient % p == 0) { quotient /= p; ++exponent; }
                if ((p & 3U) == 3U && allowed_exponent(exponent, mode)) {
                    set_bit(good, static_cast<uint64_t>(idx));
                    ++stats.exact_one_marks;
                } else {
                    // Earlier loops remove only distinct primes, so the full
                    // original p-adic part is still present in residual[idx].
...
uint64_t find_obstruction(uint64_t r, const std::vector<uint32_t>& primes, ValuationMode mode) {
    if (r == 0) return 0;
    uint64_t x = r;
    for (uint32_t p32 : primes) {
        uint64_t p = p32;
        ...
        if ((p & 3U) == 3U && allowed_exponent(exponent, mode)) return p;
    }
...
struct Args {
    ...
    ValuationMode mode = ValuationMode::ExactOne;
    bool quiet = false;
};
...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--low") a.low = std::stoull(need("--low"));
            ...
            std::string m = need("--valuation-mode");
            if (m == "exact-one") a.mode = ValuationMode::ExactOne;
            else if (m == "one-or-three") a.mode = ValuationMode::OneOrThree;
            else if (m == "any-odd") a.mode = ValuationMode::AnyOdd;
            else throw std::runtime_error("valuation mode must be exact-one, one-or-three, or any-odd");
        }
    ...
    try {
        ...
        while (cur <= args.high) {
            ...
            if (prefix > 0) {
                ...
                std::vector<uint64_t> good = classify_interval(A, B, primes, args.mode, factor_stats);
                ...
                while (word) {
                    ...
                    if (idx < len) {
                        ...
                        for (size_t si = 0; si < n_active; ++si) {
                            ++full_shift_checks;
                            ++full_factor_calls;
                            if (find_obstruction(n - shifts[si], primes, args.mode) == 0) {
                                all = false;
                                break;
                            }
                        }
        ...
        js << "{\n"
           << "  \"method\": \"exact_all_primes_odd_valuation_shifted_sieve\",\n"
           << "  \"low\": \"" << args.low << "\",\n"
           << "  \"high\": \"" << args.high << "\",\n"
           << "  \"C\": " << args.C << ",\n"
           << "  \"D\": " << args.D << ",\n"
           ...
           << "  \"valuation_mode\": \"" << mode_name(args.mode) << "\",\n"
        ...
        js << "],\n"
           ...
           << "  \"classification\": \""
           << (found.empty() ? "EXACT EXHAUSTIVE FINITE COMPUTATION FOR THE DECLARED ODD-VALUATION FAMILY"
