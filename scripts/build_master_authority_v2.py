#!/usr/bin/env python3
"""Build the deterministic A303656 Master Authority V2 candidate."""
...
ROOT='A303656_POST_TRIGGER_MASTER_AUTHORITY_V2_CANDIDATE'
...
def build(args):
    ...
    theorem['schema']='a303656-master-theorem-ledger-v2-candidate';theorem['freeze_status']='CANDIDATE_PENDING_INDEPENDENT_WEB_FREEZE_AUDIT';theorem.pop('freeze_utc',None)
    for e in theorem['entries']:
        ...
        if e['id']=='N-004':
            e.update(name='Exceptional-set spectral critical-scale and phase barrier',statement='Sparse cross-avoidance forces absolute minor-spectrum control at the critical scale; the audited exceptional-set spectral route lacks the required scale, phase alignment, or hereditary closure.',source_ids=['SRC-005'],scope_limit='This exceptional-set spectral barrier does not adjudicate every theta-series method and does not exclude a future arithmetic bipartite expansion theorem.')
    theorem['entries'] += [
    ...
    finite['schema']='a303656-master-finite-computation-ledger-v2-candidate';finite['freeze_status']='CANDIDATE_PENDING_INDEPENDENT_WEB_FREEZE_AUDIT';finite.pop('freeze_utc',None)
    ...
    routes['schema']='a303656-route-matrix-v2-candidate';routes['freeze_status']='CANDIDATE_PENDING_INDEPENDENT_WEB_FREEZE_AUDIT';routes.pop('freeze_utc',None)
    routes['routes'] += [
    ...
    sources['schema']='a303656-source-archive-inventory-v2-candidate';sources['freeze_status']='CANDIDATE_PENDING_INDEPENDENT_WEB_FREEZE_AUDIT';sources.pop('freeze_utc',None)
    ...
    defects['schema']='a303656-master-artifact-defect-ledger-v2-candidate';defects['freeze_status']='CANDIDATE_PENDING_INDEPENDENT_WEB_FREEZE_AUDIT';defects.pop('freeze_utc',None)
    defects['defects'] += [
    ...
    verdict={'schema':'a303656-master-authority-v2-candidate-verdict','master_authority_v2':'CANDIDATE — PENDING INDEPENDENT WEB FREEZE AUDIT','project':'PAUSED','active_promoted_route':'NONE','low_cost_generic_theorem_hunts':'NOT ACTIVE','targeted_restart':'ONLY ON EXPLICIT RESTART GATE','codex_large_scale_research':'NOT AUTHORIZED','codex_ultra':'NOT AUTHORIZED','a303656':'UNRESOLVED','general_proof_obtained':False,'certified_counterexample_found':False,'validator_scope':'Artifact, ledger, custody, manifest, recursive archive, and governance consistency; not independent proof validation.'}
    ...
    write(stage/'README.md',f"""# A303656 Master Authority V2 Candidate

Status: **CANDIDATE — PENDING INDEPENDENT WEB FREEZE AUDIT**.
...
""")
    report="""# Master Authority V2 candidate report
...
"""
    ...
    write(stage/'EXECUTIVE_VERDICT.md','# Executive verdict\n\nMASTER AUTHORITY V2: **CANDIDATE — PENDING INDEPENDENT WEB FREEZE AUDIT**.\n\nRepository curation and public GitHub release remain unauthorized.\n\nA303656: **UNRESOLVED**.')
    dump(stage/'environment.json',{'build_date':'2026-08-13','deterministic_zip_timestamp':'1980-01-01T00:00:00','builder':'scripts/build_master_authority_v2.py','repository_frozen_baseline':'23eecc0137cf9ddd0d0640b2ac9e1f5fa3b10c67'})

    manifest={'schema':'a303656-post-trigger-master-package-manifest-v2-candidate','package_name':ROOT,'status':verdict['master_authority_v2'],'repository_frozen_baseline':'23eecc0137cf9ddd0d0640b2ac9e1f5fa3b10c67','v1_source_sha256':sha(v1),'source_archive_count':len(sources['archives']),'source_archive_unique_sha256_count':len({x['sha256'] for x in sources['archives']}),'theorem_and_conditional_entry_count':len(theorem['entries']),'finite_computation_entry_count':len(finite['entries']),'route_count':len(routes['routes']),'defect_count':len(defects['defects']),'logical_control_defect_count':12,'phase0_physical_control_occurrence_count':48,'repaired_derivative_count':0,'scope_guards':{'new_interval_scans':0,'new_CRT_searches':0,'new_T_evaluations':0,'source_archive_reconstructions':0,'certified_baseline_overwrites':0}}
