...
A=json.loads((P/'arithmetic_audit.json').read_text())
def digits(key):return '\n'.join(textwrap.wrap(A['values'][key]['decimal'],70))
T=r'''# A303656 C=1 r=43 terminal cofactor square-part closure — Phase F
...
```
'''
for key in ('N903','N1806','C903','C1806','R1806'):T=T.replace('@@'+key+'@@',digits(key))
(P/'REPORT.md').write_text(T)
(P/'README.md').write_text('''# A303656 r=43 Phase F — reproducible evidence
...
''')
print('Wrote report bytes',len(T.encode()))
